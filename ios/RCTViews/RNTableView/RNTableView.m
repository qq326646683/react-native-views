...
- (void)createTableView {
    ...
    // 上拉加载
    if (self.footerType == 0) {
        _tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
            [self onStartLoadMore];
        }];
    } else if (self.footerType == 1) {
...
-(void)startRefreshing {
    NSLog(@"刷新-startRefreshing");
    [self.tableView.mj_header beginRefreshing];
}

-(void)stopRefreshing {
    NSLog(@"刷新-stopRefreshing");
    [self.tableView.mj_header endRefreshing];
    [self.tableView.mj_footer resetNoMoreData];
}

-(void)startLoadMore {
    NSLog(@"加载更多-startLoadMore");
    [self.tableView.mj_footer beginRefreshing];
}

-(void)stopLoadMore {
    NSLog(@"加载更多-stopLoadMore");
    [self.tableView.mj_footer endRefreshing];
}

-(void)loadLastData {
    NSLog(@"加载更多-loadLastData");
...
-(void)delAndreloadData {
    [_sections[_currentIndexPath.section][@"items"] removeObjectAtIndex:_currentIndexPath.row];
    [self.tableView reloadData];
}
