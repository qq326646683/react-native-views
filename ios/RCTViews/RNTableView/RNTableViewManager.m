...
RCT_EXPORT_VIEW_PROPERTY(headerType, float)
RCT_EXPORT_VIEW_PROPERTY(footerType, float)
    ...
    [self.bridge.uiManager addUIBlock:
     ^(__unused RCTUIManager *uiManager, NSDictionary *viewRegistry){
         ...
         if ([tableView isKindOfClass:[RNTableView class]]) {
             ...
             RCTLogError(@"Cannot startLoadMore: %@ (tag #%@) is not RNTableView", tableView, reactTag);
         }
     }];
}
    ...
    [self.bridge.uiManager addUIBlock:
     ^(__unused RCTUIManager *uiManager, NSDictionary *viewRegistry){
         ...
         if ([tableView isKindOfClass:[RNTableView class]]) {
             ...
             RCTLogError(@"Cannot stopLoadMore: %@ (tag #%@) is not RNTableView", tableView, reactTag);
         }
     }];
}

RCT_EXPORT_METHOD(loadLastData:(nonnull NSNumber *)reactTag)
{
    [self.bridge.uiManager addUIBlock:
     ^(__unused RCTUIManager *uiManager, NSDictionary *viewRegistry){
         RNTableView *tableView = viewRegistry[reactTag];
         ...
         if ([tableView isKindOfClass:[RNTableView class]]) {
             [tableView loadLastData];
         } else {
             RCTLogError(@"Cannot loadLastData: %@ (tag #%@) is not RNTableView", tableView, reactTag);
         }
     }];
}
